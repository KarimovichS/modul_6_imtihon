...
def completedtodo(request):
    results = TodoFilters(request.GET, Todo.objects.all())
    context = {
        'results': results
    }
...
def createtodo(request):
    if request.method == 'GET':
        return render(request, 'createtodo.html', {'form': todo_forms})
    else:
        try:
            ...
            return render(request, 'createtodo.html', {'form': todo_forms, 'error': 'value exceed'})


def viewtodo(request, todo_pk):
    todos = get_object_or_404(Todo, pk=todo_pk, user=request.user)
    if request.method == 'GET':
        todoform = todo_forms(instance=todos)
        return render(request, 'viewtodo.html', {'todo': todos, 'form': todoform})
    else:
        try:
            form = todo_forms(request.POST, instance=todos)
            form.save()
            return redirect('currenttodos')
        except ValueError:
